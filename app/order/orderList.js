          ...
          { orders && orders.reverse().map(order => (
            <div className="row" key={order.id}>
              ...
              <div key={order[0].selectedBooks.order_id} className="col-md-12 col-sm-12 col-lg-12">
                  ...
                  { order.map(book => (
                        <div key={book.id} className="col-xs-2 col-md-2 col-lg-2">
                          <p>{book.title}</p>
                          <img style={{"height": "100px"}}src={ book.imageUrl } />
                        </div>
                      )) }
                  <br/>
                  <h5 id="price">${order.map(book => book.price * book.selectedBooks.quantity).reduce((a, b) => a + b).toFixed(2)} Total</h5>
